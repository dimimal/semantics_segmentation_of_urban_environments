...
def m_maxpool2d(x, pool_size, strides=(1, 1), padding='valid',
    ...
    if dim_ordering == 'channels_last':
        pool_out = K.permute_dimensions(pool_out, (0, 2, 3, 1))
...
def m_maxpool3d(x, pool_size, strides=(1, 1, 1), padding='valid',
                dim_ordering='channels_first'):
    if padding == 'same':
        # TODO: add implementation for padding="same"
        raise Exception('padding="same" not supported with Theano.')
    elif padding == 'valid':
        ignore_border = True
        padding = (0, 0)
    else:
    ...
    pool_out, ind2 = m_maxpool_2d_op(input=K.permute_dimensions(output, (0, 1, 4, 3, 2)),
                                     ds=(1, pool_size[0]),
                                     st=(1, strides[0]),
                                     ignore_border=ignore_border,
                                     padding=padding)

    pool_out = K.permute_dimensions(pool_out, (0, 1, 4, 3, 2))
    ind2 = K.permute_dimensions(ind2, (0, 1, 4, 3, 2))
    ...
    if dim_ordering == 'channels_last':
        pool_out = K.permute_dimensions(pool_out, (0, 2, 3, 4, 1))
